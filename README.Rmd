...
If discrepancies are purely formatting-related (case, spacing, punctuation),
you can normalize first and use a standard join:

```{r normalize_only, message=FALSE, warning=FALSE}
library(dplyr)
library(tibble)
library(typojoin)
...
`typo_inner_join()` computes edit distances internally and deterministically
selects the best match per row:
